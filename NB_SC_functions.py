    ...
    for j in range(len(z0First_space)):
        ...
        for k in range(len(FC_space)):
            ...
            for test in run_time_space:
                ...
                cAMPi_noise = (cAMPi_noise-Nh_offset)/Nh
                cAMPi_noise_all_traces[test,:] = cAMPi_noise
                t_plot = t_plot/Nt
                cAMPi_trace_first=cAMPi_noise[stim_time_step1:int(stim_time_step1+1.5*Nt/dt)]
                cAMPi_trace_second=cAMPi_noise[stim_time_step2:int(stim_time_step2+1.5*Nt/dt)]
                PkPos1, PkProperties1 = find_peaks(cAMPi_trace_first, prominence=(prm_lims[0],prm_lims[1]))
                PkPos2, PkProperties2 = find_peaks(cAMPi_trace_second, prominence=(prm_lims[0],prm_lims[1]))
#                if PkPos1.size!=0:
#                    Pk1Pos = PkPos1[0]+stim_time_step1; 
#                    Pk1max = cAMPi_trace_first[Pk1Pos]; Pk1min = cAMPi_trace_first[Pk1Pos] -PkProperties1["prominences"][0]
#                if PkPos2.size!=0:
#                    Pk2Pos = PkPos2[0] + stim_time_step2
#                    Pk2max = cAMPi_trace_second[Pk2Pos]; Pk2min = cAMPi_trace_second[Pk2Pos] -PkProperties2["prominences"][0]
