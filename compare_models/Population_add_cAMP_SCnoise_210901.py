...
for alphafval in alphafval_arr:
    ...
    for i in range(len(t)-1):
        ...
        A_next,R_next, cAMPext_next = Sgro_pop.update(dt, time_separation,alphafval_trace[i],r[i,:])
...
dt=0.001; t_tot=30*Nt_Gregor; t=list(np.arange(0,t_tot,dt))
eta=0.002 # noise stength

ext_input_arr =  np.array([0.001,10,1000]) #np.logspace(0,3,num=5)
...
for ext_input in ext_input_arr:
    ...
    for i in range(len(t)-1):
        ...
        thetai_next, campCyto_next, campExt_next = Gregor_pop.update(dt,eta,rho,k,Vt,time_separation,ext_input_trace[i],r[i,:])
