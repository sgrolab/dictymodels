...
#create logspace
j_arr = np.linspace(0, 1, num=21) # 21 j_arr=np.array([0.5]) # 
rho_arr = np.logspace(-6, -3, num=26) # 26 rho_arr=np.array([10**(-3.5)]) # 
...
SgroPopParam={'e':e,'tauA':tauA,'tauR':tauR,'g':g,'c0':c0,'sigma':sigma,'N':N,\
            ...
            'Nt':Nt_Sgro,'Na':Nh_Sgro,'offset_A':1.5,'flux_thrs':0}

alphafval = 0 # cAMP input flow
...
def calc_updates_Sgro(j_arr, rho_arr, SgroPopParam, nSteps, index):
    ...
    A0 = -1.5*np.ones(N) # np.random.uniform(-1.5,2,N) # random initial condition 
    R0 = -0.5*np.ones(N) # np.random.uniform(-0.5,2,N) # random initial condition 
    cAMPext0 = 0
    A_trace_orig[:,0]=A0 
    R_trace_orig[:,0]=R0
    ...
    Sgro_pop_dir_cpl=Sgro2015_pop_dir_cpl(A0,R0,cAMPext0, SgroPopParam)
    for i in range(nSteps-1):
        A_next,R_next, cAMPext_next = Sgro_pop_dir_cpl.update(dt, time_separation, alphafval,dir_cpl)
...
save_name = '200414_same_init_cond_ttot_'+str(t_tot/Nt_Sgro)+'_dt'+str(dt)+'_sigma'+str(sigma)+'_dir_cpl'+str(dir_cpl)
for i in results:
    idx = i[0]; firing_rate = i[1]; height = i[2]
    pop_rate_Sgro[idx] = firing_rate
    pop_height_Sgro[idx] = height
np.savez('pop_fire_rate_Sgro_OUT_'+save_name+'.npz', j_arr = j_arr, rho_arr = rho_arr,pop_rate_Sgro = pop_rate_Sgro, pop_height_Sgro=pop_height_Sgro)
...
ax1.set_title('Sgro 2015 pop firing rate', fontdict={'fontsize': title_font_size, 'fontweight': 'medium'})
...
ax2.set_title('Sgro 2015 pop firing height, TS='+str(time_separation)+', noise = '+str(sigma),
              fontdict={'fontsize': title_font_size, 'fontweight': 'medium'})

# save image and results to the current folder
plot_name = 'pop_FR_Sgro_'+save_name+'.png'
