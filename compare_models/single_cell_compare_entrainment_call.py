...
count=0
fig3 = plt.figure(figsize=(12,6)); grid = plt.GridSpec(2, 3, wspace=0.3, hspace=0.5)
...
#            plt.show()      
            ...
            ax1.plot(t_plot_Gold,signal_trace)
            if count ==0:
                ax1.set_ylabel(r'$cAMP_{e}$ input', fontsize= 15); 
            ax1.set_title('Period '+str(period)+', stimulation\npeak width '+ str(PkWdth), fontsize = 16)
            ax1.tick_params(axis='both', which='major', labelsize=14)
            ax1.set_xlim([0, 10])
            ax2= fig3.add_subplot(grid[1:, count])
            ax2.plot(t_plot_Gold,b_trace)
            if count ==0:
                ax2.set_ylabel('Entrainment quality', fontsize= 15);
            ax2.tick_params(axis='both', which='major', labelsize=14)
            ax2.set_title('Entrainment\n quality= {:.4f}'.format(np.mean(r)), fontsize = 16)
            ax2.set_xlim([0, 10])
            fig3.text(0.5, 0.04,'Time, A.U.', ha='center', va='center',fontsize = 16) 
            plt.show() 
            count=count+1
                #             # check InitPk vs.FollowPk
                # fig3 = plt.figure(figsize=(3,3)); grid = plt.GridSpec(1, 1, wspace=0.3, hspace=0.2)
