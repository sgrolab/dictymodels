...
g = 1.6129 #0.5
#create logspace
j_arr=np.array([0]) # j_arr = np.linspace(0, 1, num=21) # 21 
rho_arr=np.array([10**(-1)]) # rho_arr = np.logspace(-5.5, -3, num=26) # 26 
...
def calc_updates_Sgro(j_arr, rho_arr, SgroPopParam, nSteps, index):
    ...
    # Check find_peaks
    fig = plt.figure()
    plt.plot(A_trace_plot[0,:])
    plt.plot(A_trace_plot[20,:])
    plt.plot(A_trace_plot[50,:])
